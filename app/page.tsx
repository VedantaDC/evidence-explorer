...
function CrossCorpusAnalysis({ mnr, other, setCorpus }:{mnr:Payload;other:OtherPayload;setCorpus:(value:Corpus)=>void}) {
  ...
  const outputByFamily=useMemo(()=>{const map=new Map<string,{standardized:string;source:string}[]>();mnr.outputs.forEach(o=>map.set(`mnr:${o.device_family_id}`,[...(map.get(`mnr:${o.device_family_id}`)||[]),{standardized:o.standardized_output,source:o.source_output_name}]));other.outputs.forEach(o=>map.set(`other:${o.family_id}`,[...(map.get(`other:${o.family_id}`)||[]),{standardized:o.standardized_output,source:o.standardized_output}]));return map},[mnr,other]);
  const table83Rows=useMemo(()=>rows.map(row=>{const outputs=row.familyIds.flatMap(id=>outputByFamily.get(id)||[]).filter(o=>outputMatchesMeasurement(row.id,`${o.standardized} ${o.source}`));const raw=unique(outputs.filter(o=>/waveform|plethysmogram|\beeg\b|\beog\b|\bemg\b|\becg\b|\bppg\b|\bpat\b/i.test(`${o.standardized} ${o.source}`)).map(o=>o.source||o.standardized));const derived=unique(outputs.filter(o=>!raw.includes(o.source||o.standardized)).map(o=>o.standardized));return {...row,rawSignals:raw,derivedFeatures:derived}}),[rows,outputByFamily]);
  ...
  return <main><CorpusNav corpus="analysis" setCorpus={setCorpus}/><ExplorerBanner context="Cross-corpus physiological and mechanistic analysis" detail="MNR + reduced-channel OLV / OLZ configurations"/>
    ...
    {subtab==="table83"&&<div className="page wide analysisPage"><div className="sectionHead"><div><h2>8.3 Cumulative FDA measurement inventory</h2><p>Actual corpus data organized by physiological parameter, with one sub-row per identified sensor technology.</p></div><div className="resultCount"><strong>{table83Rows.length}</strong> parameter–sensor rows</div></div><section className="analysisPrinciple"><strong>FDA-evidence table</strong><span>Counts describe documented capabilities. Raw-waveform wording is shown only when represented in the structured evidence; family-level outputs are not automatically attributed to a particular sensor.</span></section><div className="tableWrap table83"><table><thead><tr><th>Physiological parameter</th><th>Sensor type</th><th>No. families</th><th>Sensor location(s)</th><th>Raw / acquired signal identified</th><th>Related derived / reported features</th><th>Product codes</th></tr></thead><tbody>{table83Rows.map((row,i)=><tr key={`${row.id}-${row.sensor}-${i}`}><td><strong>{row.label}</strong></td><td><strong>{row.sensor}</strong><small>{row.aliases.join("; ")}</small></td><td><strong>{row.families}</strong><small>{row.facts} facts</small></td><td><div className="locationTags">{row.locations.map(l=><span key={l}>{l}</span>)}</div></td><td>{row.rawSignals.length?<div className="evidenceList">{row.rawSignals.map(x=><span key={x}>{x}</span>)}</div>:<span className="notEstablished">Not explicitly established in structured corpus</span>}</td><td>{row.derivedFeatures.length?<div className="featureList">{row.derivedFeatures.slice(0,12).map(x=><span key={x}>{x}</span>)}</div>:<span className="notEstablished">No related named feature identified</span>}</td><td><div className="codeTags">{row.productCodes.map(c=><span key={c}>{c}</span>)}</div></td></tr>)}</tbody></table></div><section className="analysisNotes"><h3>Interpretation rule</h3><p>Sensor-row counts overlap and do not sum to unique devices. A derived output does not prove that its raw waveform is retained, accessible, quantitative, continuous, or synchronized with other channels.</p></section></div>}
...
function EducationView({mnr,other,setCorpus}:{mnr:Payload;other:OtherPayload;setCorpus:(value:Corpus)=>void}) {
  ...
  return <main><CorpusNav corpus="education" setCorpus={setCorpus}/><ExplorerBanner context="Education: OSA physiology, measurements, and sensing" detail="Plain-language glossary with research-level distinctions"/><nav className="tabs educationTabs" aria-label="Education views"><button className={view==="mechanisms"?"active":""} onClick={()=>setView("mechanisms")}>OSA mechanisms & terms</button><button className={view==="measurements"?"active":""} onClick={()=>setView("measurements")}>Physiological measurements</button><button className={view==="sensors"?"active":""} onClick={()=>setView("sensors")}>Sensor technologies</button><button className={view==="chain"?"active":""} onClick={()=>setView("chain")}>How to read the evidence</button></nav>
    ...
    {view==="measurements"&&<div className="page educationPage"><div className="sectionHead"><div><h2>Physiological measurement glossary</h2><p>What each measurement group represents in OSA and how close it is to the underlying mechanism.</p></div><div className="resultCount"><strong>{Object.keys(measurementProfiles).length}</strong> groups</div></div><div className="educationCards">{Object.values(measurementProfiles).map(p=><article key={p.id}><div className="educationCardHead"><h3>{p.label}</h3><span className="relationship">{p.relationship}</span></div><p>{p.diseaseRole}</p><div className="mechanismTags">{p.mechanisms.map(m=><span key={m}>{m}</span>)}</div><footer>{new Set(rows.filter(r=>r.id===p.id).flatMap(r=>r.familyIds)).size} corpus families/configurations</footer></article>)}</div></div>}
    {view==="sensors"&&<div className="page wide educationPage"><div className="sectionHead"><div><h2>Sensor technology glossary</h2><p>Canonical sensor groups represented in the corpus, with their sensing principle, measurements, and documented placements.</p></div><div className="resultCount"><strong>{sensors.length}</strong> groups</div></div><div className="sensorGlossary">{sensors.map(s=><article key={s.sensor}><h3>{s.sensor}</h3><p>{s.definition}</p><h4>Measures</h4><div className="mechanismTags">{s.measurements.map(m=><span key={m}>{m}</span>)}</div><h4>Documented locations</h4><div className="locationTags">{s.locations.map(l=><span key={l}>{l}</span>)}</div></article>)}</div></div>}
...
export default function Home() {
  ...
  if (corpus === "education") return <EducationView mnr={data} other={otherData} setCorpus={setCorpus}/>;
