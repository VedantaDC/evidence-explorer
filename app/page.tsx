...
function CrossCorpusAnalysis({ mnr, other, setCorpus, navigateEducation }:{mnr:Payload;other:OtherPayload;setCorpus:(value:Corpus)=>void;navigateEducation:(target:EducationTarget)=>void}) {
  ...
  const measurementOptions=unique(rows.map(r=>r.label)); const sensorOptions=unique(rows.map(r=>r.sensor)); const locationOptions=unique(rows.flatMap(r=>r.locations));
  ...
  const inventoryGroups=Object.values(measurementProfiles).map(profile=>({
    ...profile,
    sensorRows:filtered.filter(row=>row.id===profile.id).map(row=>{
      const directRecords=dedupeEvidence(sensorEvidence.filter(e=>e.profileId===profile.id&&e.sensor===row.sensor).map(e=>e.record));
      const signal=highConfidenceAcquiredSignal(profile.id,row.sensor);
      const rawSignals=signal?[countedEvidence(signal,directRecords)]:[];
      const familySet=new Set(row.familyIds);
      const related=outputEvidence.filter(e=>familySet.has(e.record.familyKey)&&e.profileIds.includes(profile.id)&&isHighConfidenceReportedFeature(e.standardized));
      const derivedFeatures=unique(related.map(e=>e.standardized)).map(label=>countedEvidence(label,related.filter(e=>sameLabel(e.standardized,label)).map(e=>e.record))).filter(item=>item.familyCount>0).sort((a,b)=>b.familyCount-a.familyCount||a.label.localeCompare(b.label));
      return {...row,evidenceRecords:directRecords,rawSignals,derivedFeatures};
    }).sort((a,b)=>otherLast(a.sensor)-otherLast(b.sensor)||b.families-a.families||a.sensor.localeCompare(b.sensor)),
  })).filter(group=>group.sensorRows.length).sort((a,b)=>otherLast(a.label)-otherLast(b.label)||a.label.localeCompare(b.label));
  const profileFamilyCount=(id:string)=>new Set(rows.filter(r=>r.id===id).flatMap(r=>r.familyIds)).size; const allOutputText=[...mnr.outputs.filter(o=>isPhysiologicalOutputFact(o.standardized_output)).map(o=>`${o.standardized_output} ${o.source_output_name}`),...other.outputs.filter(o=>isPhysiologicalOutputFact(o.standardized_output)).map(o=>o.standardized_output)]; const countOutput=(pattern:RegExp)=>allOutputText.filter(x=>pattern.test(x)).length;
  ...
  return <main><CorpusNav corpus="analysis" setCorpus={setCorpus}/><ExplorerBanner context="Cross-corpus physiological and mechanistic analysis" detail="MNR + comprehensive OLV / OLZ PSG and sleep-acquisition corpus"/>
    ...
    {subtab==="inventory"&&<div className="page wide analysisPage"><div className="sectionHead"><div><h2>Physiological measurement → sensor → location</h2><p>A consolidated view: each physiological measurement appears once, with its sensor technologies, documented locations, acquired signals, derived physiological features, and supporting FDA evidence underneath.</p></div><div className="resultCount"><strong>{inventoryGroups.length}</strong> measurements · {filtered.length} sensor rows</div></div><section className="analysisPrinciple compactPrinciple"><strong>High-confidence only</strong><span>Acquired signals are shown only when the documented sensor and measured parameter establish them directly. Derived features are limited to explicit reported outputs in the same device families; otherwise the table says “not explicitly established.”</span></section><section className="filterPanel cleanInventoryFilters"><label className="filter search"><span>Search measurement, sensor, or location</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="e.g., airflow, thermistor, finger"/></label><Select label="Measurement" value={measurement} options={measurementOptions} onChange={setMeasurement}/><Select label="Sensor type" value={sensorFilter} options={sensorOptions} onChange={setSensorFilter}/><Select label="Location" value={location} options={locationOptions} onChange={setLocation}/><Select label="Product code" value={productCode} options={["MNR","OLV","OLZ"]} onChange={setProductCode}/><button className="clear" onClick={clear}>Clear</button></section><div className="cleanInventoryGroups">{inventoryGroups.map(group=><section className="cleanInventoryGroup" key={group.id}><header><div><button className="inventoryMeasurementLink" onClick={()=>navigateEducation({view:"measurements",id:group.id})}>{group.label}</button><span>{group.relationship}</span></div><strong>{group.sensorRows.length} sensor type{group.sensorRows.length===1?"":"s"}</strong></header><div className="tableWrap cleanInventoryTable"><table><thead><tr><th>Sensor technology</th><th>Documented location(s)</th><th>High-confidence acquired signal</th><th>Explicit derived physiological feature(s)</th><th>FDA corpus evidence</th></tr></thead><tbody>{group.sensorRows.map(row=><tr key={`${group.id}-${row.sensor}`}><td><button className="inlineEducationLink" onClick={()=>navigateEducation({view:"sensors",id:anchorId(row.sensor)})}>{row.sensor}</button>{row.aliases.length>1&&<details><summary>Source wording</summary><div className="aliasList">{row.aliases.map(a=><span key={a}>{a}</span>)}</div></details>}</td><td><div className="locationTags">{row.locations.map(l=><span key={l}>{l}</span>)}</div></td><td>{renderEvidenceItems(row.rawSignals,"raw")}</td><td>{renderEvidenceItems(row.derivedFeatures,"derived")}</td><td><button className="inventoryEvidenceButton" onClick={()=>openEvidence(`${group.label} · ${row.sensor}`,`Supporting FDA records for this physiological measurement–sensor combination: N=${row.families} families/configurations and ${row.evidenceRecords.length} linked 510(k)s.`,row.evidenceRecords)}><strong>N={row.families} families</strong><span>{row.evidenceRecords.length} linked 510(k)s · view evidence</span></button><div className="codeTags">{row.productCodes.map(c=><span key={c}>{c}</span>)}</div><div className="kLinks inventoryKLinks">{row.evidenceRecords.slice(0,3).map(record=><a key={`${record.kNumber}-${record.sourceUrl}`} href={record.sourceUrl} target="_blank" rel="noreferrer" title={`Open ${record.familyName} FDA evidence`}>{record.kNumber}</a>)}{row.evidenceRecords.length>3&&<button onClick={()=>openEvidence(`${group.label} · ${row.sensor}`,`All ${row.evidenceRecords.length} linked 510(k)s supporting this row.`,row.evidenceRecords)}>+{row.evidenceRecords.length-3} more</button>}</div></td></tr>)}</tbody></table></div></section>)}</div>{!inventoryGroups.length&&<section className="emptyState"><h3>No matching measurement–sensor rows</h3><p>Clear one or more filters to broaden the table.</p></section>}{drilldown&&<section className="evidenceDrilldown" aria-live="polite"><div className="drilldownHead"><div><span className="eyebrow dark">FILTERED EVIDENCE LIST</span><h3>{drilldown.title}</h3><p>{drilldown.note}</p></div><button onClick={()=>setDrilldown(null)} aria-label="Close filtered evidence list">×</button></div><div className="evidenceFamilies">{[...new Map(drilldown.records.map(record=>[record.familyKey,record])).values()].map(family=><article key={family.familyKey}><h4>{family.familyName}</h4><span>{family.productCode}</span><div className="kLinks">{drilldown.records.filter(record=>record.familyKey===family.familyKey).map(record=><a key={`${record.kNumber}-${record.sourceUrl}`} href={record.sourceUrl} target="_blank" rel="noreferrer" title="Open FDA 510(k) evidence">{record.kNumber}</a>)}</div></article>)}</div></section>}<section className="analysisNotes"><h3>Evidence counts</h3><p><strong>N</strong> is the number of unique device families/configurations supporting that measurement–sensor row. Every populated signal or derived feature is clickable and opens its supporting evidence; the FDA evidence column links the source 510(k)s for the sensor row itself.</p></section></div>}
